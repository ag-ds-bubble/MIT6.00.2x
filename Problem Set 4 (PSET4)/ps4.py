...
def r_squared(y, estimated):
    ...
    """
    y,yhat = np.array(y), np.array(estimated)
    ymean = np.mean(y)
    sse = sum((y-yhat)**2)
...
def evaluate_models_on_training(x, y, models):
    ...
    for emodel in models:
        estimated = np.polyval(emodel, x)
        _r2 = r_squared(y,estimated)
        if r_select<_r2:
            r_select = _r2
            selected_model = emodel
        # Plot
        plt.scatter(x,y, label='Actual', color='b')
        plt.plot(x,estimated,color='r',label='Estimated')
        plt.title(f'Actual v/s Fitted @ Model R2 = {_r2}')
        plt.xlabel('Year')
